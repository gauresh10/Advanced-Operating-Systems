...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq) {
    ...
    runqueue_t *runq, *runqnew;
    prio_struct_t *prioq;
    uthread_head_t *u_head;
    uthread_struct_t *u_obj;
    //changes
    kthread_context_t *k_ctx;
    kthread_runqueue_t *kthread_runq_new;
    ...
    if(sched_policy==0) {
        ...
        unsigned int i = 0;
        // lock current k thread
        gt_spin_lock(&(kthread_runq->kthread_runqlock));
        runq = kthread_runq->active_runq;
        kthread_runq->kthread_runqlock.holder = 0x04;
        unsigned int switch_count = kthread_runq->switch_num;


        // Performance
        ksched_shared_info_t *ksched_info = &ksched_shared_info;
        //if(ksched_info->kthread_cur_uthreads > 20)
        {
            if (!(runq->uthread_mask) && (kthread_create_flag == 1)) { /* No jobs in active. search other runqueue */
                assert(!runq->uthread_tot);
                //unlock current kthread runqueue
                gt_spin_unlock(&(kthread_runq->kthread_runqlock));

                for (i = 0; i < num_cpus; i++) {
                    if (i == kthread_apic_id())
                        continue;

                    k_ctx = kthread_cpu_map[i];
                    kthread_runq_new = &(k_ctx->krunqueue);
                    //lock current kthread

                    gt_spin_lock(&(kthread_runq_new->kthread_runqlock));

                    if (switch_count >= kthread_runq_new->switch_num)
                    {

                        kthread_runq->switch_num++;

                        runqnew = kthread_runq_new->active_runq;
                        kthread_runq_new->kthread_runqlock.holder = 0x04;

                        if (!(runqnew->uthread_mask)) {
                            gt_spin_unlock(&(kthread_runq_new->kthread_runqlock));
                            continue;
                        } else {
                            kthread_runq_new->kthread_runqlock.holder = 0x04;
                            uprio = LOWEST_BIT_SET(runqnew->uthread_mask);
                            prioq = &(runqnew->prio_array[uprio]);
                            ...
                            u_head = &(prioq->group[ugroup]);
                            u_obj = TAILQ_FIRST(u_head);
                            __rem_from_runqueue(runqnew, u_obj);
