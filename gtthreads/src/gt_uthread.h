...
typedef struct uthread_struct
{
	...
	//CHANGE
	float credit_score;//credit value
	unsigned int credit_flag;// 0 for underutilized and 1 for over utilized
    unsigned int init_credit_score;
    // calculate spu time
    struct timeval start;
    struct timeval end;

    unsigned int long time_slice;

    unsigned int long total_uthread_time_slice;
