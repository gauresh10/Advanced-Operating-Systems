...
#endif
    thread_data[ptr->tid]= (tv2.tv_sec * 1000000 + tv2.tv_usec) - (tv1.tv_sec * 1000000 + tv1.tv_usec);
...
int main(int argc, char **argv) {
    ...
    if (sched_policy == 0) {
        ...
        for (inx = 0; inx < NUM_THREADS; inx++) {
            ...
            uthread_create(&utids[inx], uthread_mulmat, uarg, uarg->gid, 50);
        }
        ...
        for (j = 0; j < 4; j++) { //credits
            ...
            for (k = 0; k < 4; k++) {//matrix size
                ...
                for (inx = 0; inx <  8; inx++) {
                    ...
                    uthread_create(&utids1[thread_id], uthread_mulmat1, uarg, uarg->gid, credits[j]);
                    thread_id++;
