...
/** DECLARATIONS **/
/**********************************************************************/
//CHANGES CREDIT VALUE

extern unsigned int total_exe_time;
...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, uthread_group_t u_gid,unsigned int credit_assigned);
...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
	...
	if((u_obj = kthread_runq->cur_uthread)) {
        ...
        if(sched_policy==1) {
            //end timer here
            gettimeofday(&u_obj->end, NULL);
            u_obj->time_slice = ((u_obj->end.tv_sec) - (u_obj->start.tv_sec)) * 1000000 +
                                ((u_obj->end.tv_usec) - (u_obj->start.tv_usec));


            u_obj->total_uthread_time_slice+=u_obj->time_slice;

            float frac = (float) (u_obj->time_slice / (KTHREAD_VTALRM_USEC));

            prop_schedule = frac * credit_val;
		...
		if(u_obj->uthread_state & (UTHREAD_DONE | UTHREAD_CANCELLED)){
            ...
            thread_cpu_time[u_obj->uthread_tid].cpu_time = u_obj->total_uthread_time_slice;
			...
			if(sched_policy == 0 ) {
				...
				//overutilized task, put in expire queue
				if(u_obj->credit_flag==1){
                    //printf("\n\t over utilized\n\t");
                    //credit sub here
                    u_obj->credit_score =u_obj->init_credit_score;
                    u_obj->credit_flag=0;
					add_to_runqueue(kthread_runq->expires_runq, &(kthread_runq->kthread_runqlock), u_obj);
					/* XXX: Save the context (signal mask not saved) */
				}
				else if(u_obj->credit_flag==0){ // under utilized task, put in current run queue
                  //  printf("\n\t under utilized\n\t");
                    if(prop_schedule > credit_val){
                        u_obj->credit_score =u_obj->credit_score -credit_val;
                    }
                    else {
                        u_obj->credit_score = u_obj->credit_score - prop_schedule;
                    }

                    if(u_obj->credit_score < 1) {
                        u_obj->credit_flag = 1;
                    }
...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, uthread_group_t u_gid,unsigned int credit_assigned)
{
	...
	//CHANGE
	u_new->credit_flag = 0;
	//CHANGE the credit associated with this
	u_new->credit_score = credit_assigned;
    u_new->init_credit_score = credit_assigned;
